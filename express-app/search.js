...
app.get('/search', async (req, res) => {
    const { subject, location, price, spaces } = req.query;
    ...
    try {
      ...
      if (subject) {
        filter.subject = { $regex: subject, $options: 'i' }; // Case-insensitive regex search
      }
      if (location) {
        filter.location = { $regex: location, $options: 'i' };
      }
      if (price) {
        filter.price = parseFloat(price); // Assuming price is a numeric field
      }
      if (spaces) {
        filter.spaces = parseInt(spaces); // Assuming spaces is a number
      }
  
      // Fetch lessons based on filter
